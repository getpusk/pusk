...
if(ev.type==='channel_message_delete'){const mid=d.message_id;const el=document.getElementById('m-'+mid);if(el)el.remove();document.querySelectorAll('.m[data-reply="'+mid+'"]').forEach(r=>{const q=r.querySelector('.m-quote');if(q)q.innerHTML='<i>'+(S.lang==='ru'?'Удалённое сообщение':'Deleted message')+'</i>'})}
...
if(ev.type==='role_update'){import('./storage.js').then(m=>{m.set('role',d.role)});import('./views.js').then(m=>{m.showList()});toast(S.lang==='ru'?'\u0420\u043e\u043b\u044c \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u0430: '+d.role:'Role changed: '+d.role)}
if(ev.type==='user_status'){const ud=ev.payload;if(ud&&ud.username){document.querySelectorAll('.m-ava[data-uname="'+ud.username+'"]').forEach(el=>{el.classList.remove('online','away');if(ud.status==='online')el.classList.add('online');else if(ud.status==='away')el.classList.add('away')});import('./util.js').then(u=>{const api=u.api;api('GET','/api/online').then(r=>{if(!r)return;const us=r.users||[];const on=us.filter(x=>x.status==='online');const aw=us.filter(x=>x.status==='away');const mob=window.innerWidth<600;let s='';if(mob){if(on.length)s=on.length+'\u25cf';if(aw.length)s+=(s?' ':'')+'\u25cb'+aw.length}else{if(on.length)s=on.map(x=>x.username).join(', ');if(aw.length)s+=(s?' \xb7 ':'')+(S.lang==='ru'?'\u043e\u0442\u043e\u0448\u043b\u0438: ':'away: ')+aw.map(x=>x.username).join(', ')}$('hdr-online').textContent=s})})}}
if(ev.type==='channel_rename'){const ch=S.channels&&S.channels.find(c=>c.id===d.channel_id);if(ch)ch.name=d.new_name;if(S.curChan===d.channel_id)$('hdr-title').textContent='# '+d.new_name}
}}
