...
$('file-caption').addEventListener('input',function(){if(!S.curChan)return;const v=this.value;const cursor=this.selectionStart;const before=v.substring(0,cursor);const atMatch=before.match(/@([\p{L}\p{N}_]*)$/u);if(atMatch){const query=atMatch[1].toLowerCase();const me=get('uname')||'';const matches=S.mentionUsers.filter(u=>u.username.toLowerCase().startsWith(query)&&u.username!==me);if(matches.length>0){const ml=$('mention-list');ml.innerHTML='';matches.slice(0,8).forEach(u=>{const item=document.createElement('div');item.className='ac-item-user';item.dataset.mention=u.username;item.dataset.target='file-caption';item.textContent=u.username;ml.appendChild(item)});ml.style.display='block';_mentionIdx=-1;return}}$('mention-list').style.display='none'});
};
...
$('msg-in').addEventListener('input',function(){const v=this.value;const cursor=this.selectionStart;const before=v.substring(0,cursor);
  ...
  if(S.curChan){const atMatch=before.match(/@([\p{L}\p{N}_]*)$/u);if(atMatch){const query=atMatch[1].toLowerCase();const me=get('uname')||'';const matches=S.mentionUsers.filter(u=>u.username.toLowerCase().startsWith(query)&&u.username!==me);if(matches.length>0){const ml=$('mention-list');ml.innerHTML='';matches.slice(0,8).forEach(u=>{const item=document.createElement('div');item.className='ac-item-user';item.dataset.mention=u.username;item.textContent=u.username;ml.appendChild(item)});ml.style.display='block';_mentionIdx=-1;return}}}$('mention-list').style.display='none';
...
// ── Callbacks & Delete ──
async function onCb(el){if(S.curChan){await api('POST',`/api/channels/${S.curChan}/ack`,{action:el.dataset.cb,message_id:parseInt(el.dataset.mid)});const u=get('uname')||'?';const tm=new Date().toLocaleTimeString(S.lang==='ru'?'ru':'en',{hour:'2-digit',minute:'2-digit'});el.closest('.m-kb').innerHTML=`<span class="ack-result">${el.textContent} @${u} ${tm}</span>`;toast(el.textContent+' \u2713');const msg=el.closest('.m');if(msg){msg.classList.remove('m-alert');msg.classList.add('m-acked')}}else if(S.curChat){el.disabled=true;el.style.opacity='0.5';await api('POST',`/api/chats/${S.curChat}/callback`,{data:el.dataset.cb,message_id:parseInt(el.dataset.mid)})}}
