...
function renderOrgSwitch(){const el=$('s-org-switch');const orgs=JSON.parse(localStorage.getItem('pusk_orgs')||'{}');const cur=localStorage.getItem('pusk_org')||'default';const keys=Object.keys(orgs);if(keys.length<=1){el.innerHTML='';return}el.innerHTML='<div class="s-label">'+t('orgs_title')+'</div>'+keys.map(k=>{const o=orgs[k];return`<button class="s-btn s-org-btn${k===cur?' active':''}" onclick="window.switchOrg('${k}')"><b>${k}</b> <span class="s-org-user">(${o.user||'?'})</span>${k===cur?' ✓':''}</button>`}).join('')}
