...
async function togglePush(){if((get('org')||'default')==='default'){toast(S.lang==='ru'?'Push доступен только в организации. Создайте организацию в настройках.':'Push available only in organizations. Create one in settings.');return}
  if(Notification.permission==='denied'){
    $('s-push-btn').textContent=S.lang==='ru'?'\ud83d\udd12 Блок':'\ud83d\udd12 Off';
  ...
  }else{Notification.requestPermission().then(p=>{if(p==='granted'){registerPush();$('s-push-btn').textContent=S.lang==='ru'?'Push: Вкл ✓':'Push: On ✓'}else{$('s-push-btn').textContent=S.lang==='ru'?'\ud83d\udd12 Блок':'\ud83d\udd12 Off'}})}}
