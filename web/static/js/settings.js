...
async function renderSettings(){S.inviteUrl='';const ir=$('s-invite-result');if(ir)ir.textContent='';const u=get('uname')||'?';const o=get('org')||'default';$('s-profile').textContent=u+' @ '+o;
  if(get('role')==='admin'){api('GET','/api/stats').then(st=>{if(st&&st.users){const mb=(st.file_size/1048576).toFixed(1);$('s-profile').textContent=u+' @ '+o+' · '+st.users+(S.lang==='ru'?' польз':' users')+' · '+st.messages+(S.lang==='ru'?' сообщ':' msgs')+' · '+mb+' MB'}})}const h=await api('GET','/api/health');$('s-about').innerHTML='Pusk '+(h.version||'?')+' <a href="https://github.com/getpusk/pusk" target="_blank" class="s-about-link">GitHub</a> <a href="https://github.com/getpusk/pusk#readme" target="_blank" class="s-about-link">Docs</a> <a href="https://getpusk.ru" target="_blank" class="s-about-link">'+(S.lang==='ru'?'О Pusk':'About')+'</a>';if((get('org')||'default')==='default'){$('s-push-btn').textContent=S.lang==='ru'?'Создайте организацию':'Create org';$('s-push-btn').style.opacity='0.5';
