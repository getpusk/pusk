...
async function renderSettings(){S.inviteUrl='';const ir=$('s-invite-result');if(ir)ir.textContent='';const u=get('uname')||'?';const o=get('org')||'default';$('s-profile').textContent=u+' @ '+o;
...
}else if(!window.isSecureContext){$('s-push-btn').textContent=S.lang==='ru'?'Нужен HTTPS':'HTTPS required';$('s-push-btn').style.opacity='0.5'}else{(async()=>{let pushLabel;if(Notification.permission==='denied'){pushLabel=S.lang==='ru'?'Push: Выкл':'Push: Off'}else if(Notification.permission==='granted'&&'serviceWorker' in navigator){try{const r=await navigator.serviceWorker.ready;const s=await r.pushManager.getSubscription();pushLabel=s?(S.lang==='ru'?'Push: Вкл ✓':'Push: On ✓'):(S.lang==='ru'?'Push: Выкл':'Push: Off')}catch{pushLabel=S.lang==='ru'?'Push: Вкл ✓':'Push: On ✓'}}else{pushLabel=S.lang==='ru'?'Подключить':'Connect'}$('s-push-btn').textContent=pushLabel})();$('s-push-btn').style.opacity='1';const db=$('s-demo-banner');if(db)db.remove()}
  ...
  {const oldPwd=$("s-change-pwd");if(oldPwd)oldPwd.remove();if((get("org")||"default")!=="default"){const p=document.createElement("button");p.id="s-change-pwd";p.className="s-btn";p.style.cssText="width:100%;font-size:12px;padding:6px;margin-top:4px;color:var(--text2)";p.textContent=S.lang==="ru"?"Сменить пароль":"Change password";p.onclick=async()=>{const o=prompt(S.lang==="ru"?"Текущий пароль:":"Current password:");if(!o)return;const n=prompt(S.lang==="ru"?"Новый пароль (мин. 6):":"New password (min 6):");if(!n||n.length<8){toast(S.lang==="ru"?"Минимум 6 символов":"Min 6 chars");return}const r=await api("POST","/api/change-password",{old_pin:o,new_pin:n});if(r.ok)toast(S.lang==="ru"?"Пароль изменён":"Password changed");else toast(r.error||"Error")};$("s-out").before(p)}}
}

async function togglePush(){if(!window.isSecureContext){$('s-push-btn').textContent=S.lang==='ru'?'Нужен HTTPS':'HTTPS required';toast(S.lang==='ru'?'Push уведомления требуют HTTPS. Настройте TLS сертификат (напр. Caddy, nginx + Let\'s Encrypt).':'Push notifications require HTTPS. Configure TLS (e.g. Caddy, nginx + Let\'s Encrypt).');return}if((get('org')||'default')==='default'){toast(S.lang==='ru'?'Push доступен только в организации. Создайте организацию в настройках.':'Push available only in organizations. Create one in settings.');return}
...
function renderOrgSwitch(){const el=$('s-org-switch');const orgs=getJSON('orgs')||{};const cur=get('org')||'default';const keys=Object.keys(orgs).filter(k=>k!=='default');
  ...
  addBtn.onclick=()=>{$('settings').style.display='none';$('settings-bg').style.display='none';$('org-slug').value='';$('org-name').value='';$('org-user').value='';$('org-pin').value='';$('org-msg').textContent='';$('org-modal-bg').classList.add('open');history.pushState(null,'',location.href);$('org-slug').focus()};
