...
// ── Messages ──
export function addMsg(m){const el=$('msgs');const emptyEl=el.querySelector('#msgs-empty');if(emptyEl)emptyEl.remove();
  ...
  const who=m.sender==='bot'?(m.bot_name||$('hdr-title').textContent.replace('# ','')||'Bot'):(m.sender_name||'?');const tm=fmtTime(m.date);
  ...
  wrap.dataset.mine=isMine?'1':'0';if(m.reply_to)wrap.dataset.reply=m.reply_to;

  const ava=document.createElement('div');
  ava.className='m-ava';
  if(m.sender==='user'&&m.sender_name)ava.dataset.uname=m.sender_name;
...
export async function showList(){try{S.curChat=null;S.curChan=null;S.mentionUsers=[];if(S.ws&&S.ws.readyState===1)S.ws.send(JSON.stringify({type:'viewing',channel_id:0}));S.replyToId=0;$('reply-bar').style.display='none';$('pinned-bar').innerHTML='';$('typing-bar').style.display='none';remove('view');$('main-list').style.display='block';$('chat-view').style.display='none';$('input-bar').style.display='none';$('hdr-back').style.display='none';$('hdr-title').textContent=t('main');$('hdr-online').textContent='';const isGuest=S.isDemo||get('uname')==='guest';const isAdmin=get('role')==='admin';$('fab').style.display=(isGuest||!isAdmin)?'none':'flex';
  const el=$('main-list');
  showLoading(el);
  ...
  if (!get('hints_dismissed_' + orgSlug)) {
    ...
    hints.querySelectorAll('.hint-card').forEach(card => {
        card.onclick = () => {
            const action = card.dataset.action;
            if (action === 'general') openChan(+card.dataset.id, 'general');
            else if (action === 'create') { const fab = $('fab'); if(fab) fab.click(); }
            else if (action === 'invite') {
                const ava = $('hdr-ava'); if(ava) ava.click();
                setTimeout(() => { const inv = $('s-invite'); if(inv) inv.click(); }, 300);
            }
            else if (action === 'push') {
                const ava = $('hdr-ava'); if(ava) ava.click();
                setTimeout(() => { const pb = $('s-push-btn'); if(pb) pb.click(); }, 300);
            }
        };
    });
  }
...
export async function openChan(chId,name){try{if(S.curChan===chId&&$('chat-view').style.display==='flex')return;if(!name&&S.channels){const ch=S.channels.find(c=>c.id===chId);if(ch)name=ch.name}S.lastMsgDate='';setJSON('view',{t:'ch',id:chId,n:name});history.pushState(null,'',location.href);$('fab').style.display='none';S.curChan=chId;if(S.ws&&S.ws.readyState===1)S.ws.send(JSON.stringify({type:'viewing',channel_id:chId}));S.curChat=null;S.replyToId=0;$('reply-bar').style.display='none';$('typing-bar').style.display='none';$('main-list').style.display='none';$('chat-view').style.display='flex';$('input-bar').style.display='flex';$('hdr-back').style.display='inline';$('hdr-title').textContent='# '+name;$('msg-in').placeholder=(S.lang==='ru'?'Написать в #':'Write in #')+name+'...';api('GET','/api/online').then(r=>{if(!r)return;const us=r.users||[];if(!us.length){$('hdr-online').textContent='';return}const on=us.filter(u=>u.status==='online');const aw=us.filter(u=>u.status==='away');const mob=window.innerWidth<600;let s='';if(mob){if(on.length)s=on.length+'\u25cf';if(aw.length)s+=(s?' ':'')+'\u25cb'+aw.length}else{if(on.length)s=on.map(u=>u.username).join(', ');if(aw.length)s+=(s?' \xb7 ':'')+(S.lang==='ru'?'\u043e\u0442\u043e\u0448\u043b\u0438: ':'away: ')+aw.map(u=>u.username).join(', ')}$('hdr-online').textContent=s;updateAvatarDots(us)});api('GET','/api/users').then(r=>{if(r&&Array.isArray(r))S.mentionUsers=r});const msgs=await api('GET',`/api/channels/${chId}/messages`);$('msgs').innerHTML='';const chInfo=S.channels.find(c=>c.id===chId);const pinnedId=chInfo?chInfo.pinned_message_id:0;$('pinned-bar').innerHTML='';if(msgs&&msgs.length){msgs.reverse().forEach(m=>{if(!m.sender)m.sender='bot';addMsg(m)});if(pinnedId>0){const pinMsg=msgs.find(m=>m.message_id===pinnedId);if(pinMsg){renderPinBar(pinnedId,pinMsg.text||'')}}}else if(msgs&&msgs.error){$('msgs').innerHTML=`<div class="m-empty" id="msgs-empty">${msgs.error}</div>`}else $('msgs').innerHTML=`<div class="m-empty" id="msgs-empty">${t('no_msg')}</div>`;
  ...
  scrollDown();if(S.curChan===chId)api('POST',`/api/channels/${chId}/mark-read`);$('msg-in').focus();api('GET',`/api/channels/${chId}/readers`).then(readers=>{if(S.curChan!==chId)return;if(!readers||!readers.length)return;const myName=get('uname');const lastMsgId=msgs&&msgs.length?msgs[msgs.length-1].message_id:0;const whoRead=readers.filter(r=>r.last_read_id>=lastMsgId&&r.username!==myName);if(whoRead.length>0){const names=whoRead.map(r=>r.username).join(', ');const readDiv=document.createElement('div');readDiv.className='read-receipt';readDiv.textContent=(S.lang==='ru'?'\u041f\u0440\u043e\u0447\u0438\u0442\u0430\u043d\u043e: ':'Read by: ')+names;$('msgs').appendChild(readDiv)}})}catch(e){console.error('[pusk] openChan error:',e);$('msgs').innerHTML='<div class="m-empty">Error loading channel. <span style="cursor:pointer;color:var(--accent)" onclick="location.reload()">Reload</span></div>'}}


// ── Online dots on avatars ──
export function updateAvatarDots(users){if(!users)return;const map={};users.forEach(u=>{if(u.username)map[u.username]=u.status});document.querySelectorAll('.m-ava[data-uname]').forEach(el=>{const st=map[el.dataset.uname];el.classList.remove('online','away');if(st==='online')el.classList.add('online');else if(st==='away')el.classList.add('away')})}
...
let _botLong=null;
$('main-list').addEventListener('touchstart',e=>{const bn=e.target.closest('.bot-name');if(!bn||get('role')!=='admin')return;e.preventDefault();const row=bn.closest('.bot-row');if(!row)return;_botLong=setTimeout(()=>{const bid=+row.dataset.botId;const oldName=row.dataset.botName;const name=prompt(S.lang==='ru'?'\u041d\u043e\u0432\u043e\u0435 \u0438\u043c\u044f \u0431\u043e\u0442\u0430:':'New bot name:',oldName);if(!name||name===oldName)return;api('PUT','/admin/bots/'+bid,{name}).then(r=>{if(r&&r.ok){toast(S.lang==='ru'?'\u041f\u0435\u0440\u0435\u0438\u043c\u0435\u043d\u043e\u0432\u0430\u043d\u043e':'Renamed');showList()}else{toast(r&&r.error||'Error')}})},800)});
$('main-list').addEventListener('touchend',e=>{if(_botLong){clearTimeout(_botLong);_botLong=null}});
$('main-list').addEventListener('touchmove',e=>{if(_botLong){clearTimeout(_botLong);_botLong=null}});
