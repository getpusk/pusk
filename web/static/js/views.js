...
async function renameCurrentBot(){if(!S.curBotId||get('role')!=='admin')return;const oldName=$('hdr-title').textContent;const name=prompt(S.lang==='ru'?'Новое имя бота:':'New bot name:',oldName);if(!name||name===oldName)return;const r=await api('PUT','/admin/bots/'+S.curBotId,{name});if(r&&r.ok){$('hdr-title').textContent=name;setJSON('view',{t:'chat',id:S.curBotId,n:name});toast(S.lang==='ru'?'Переименовано':'Renamed')}else{toast(r&&r.error||'Error')}}
